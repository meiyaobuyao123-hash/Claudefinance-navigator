...
flutter test test/models/ test/logic/   # 单元测试（80/80）
...
**待实现（Agent v2）**：
- M07 护栏 → M03 分层 Prompt → M02 持仓注入 → M01 冷启动（Phase 1）
- M06 流式 → M04 状态机 → M05 Tool Use（Phase 2）
- M09 Token 优化（Phase 3）
- M08 评估反馈（Phase 4）
...
| `lib/features/ai_chat/presentation/pages/ai_chat_page.dart` | AI 对话核心（当前 v1，待 v2 重构）|
