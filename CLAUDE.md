...
- HTTP：Dio
- AI 模型：`claude-sonnet-4-20250514`（Claude API，主key + 备用key 三级降级）+ `deepseek-chat`（DeepSeek API，兜底）
...
- ✅ **AI API 切换为 Claude 默认 + DeepSeek 备用**
  - `_callClaude()`：Anthropic Messages API，x-api-key 头，model claude-sonnet-4-20250514
  - `_callDeepSeek()`：OpenAI 兼容格式，Bearer auth
  - 自动降级：Claude 失败 → 回退 DeepSeek，对用户透明
  - AppBar 副标题改为"由 Claude AI 驱动"
- ✅ **产品操作预览截图系统**
  - `PlatformScreenshot` 模型：platformId/step/imageAsset/caption
  - 产品详情页「平台操作预览」区域展示招行等平台的实际操作步骤
  - 已上传截图：定期存款（3张）、大额存单（3张）
  - 移除无图片文件的空白占位截图，只保留有实际图片的条目
- ✅ **App 图标更新**：深蓝渐变背景 + 白色圆润"财"字 + 金色上升趋势线箭头
- ✅ **App Store 提审 Build 2**（2026-03-16）
...
4. AI 流式输出（Streaming SSE），减少等待感
5. 基金/股票调仓记录流水（每次加减仓写入 trade_log 表）
6. 股票持仓走势图（stock_snapshots 表，与基金快照同步记录）
...
| `lib/core/config/api_keys.dart` | ⚠️ Claude + DeepSeek Key（gitignored，本地才有） |
| `ios/ExportOptions.plist` | App Store 上传导出配置（手动签名） |
...
| AI 模型 | Claude claude-sonnet-4-20250514（默认）+ DeepSeek deepseek-chat（备用降级） |
