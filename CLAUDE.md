...
- ✅ 资产配置评估器：3种维度切换 + 评分 + 风险提示 + 优化建议
  - 📊 按属性：流动/稳健/增值/另类/高弹性（原有5类）
  - 🌍 按地区：大陆/香港/美元区/加密/现金（地理分散分析）
  - 🏷️ 按类型：现金货基/固定收益/权益/保险/贵金属/加密（品类穿透）
  - 每种维度独立保存配置比例，切换时不丢失数据，评估逻辑各自独立
...
2. 资产评估器：滑块交互升级（数字输入 → 滑块）+ 历史报告保存
3. 产品导航页：加 DeepLink 实测（测试"去购买"按钮是否正常跳转）
